...
export default function MateriaCard({ materia }: { materia: materia }) {
    ...
    return (
        <Pressable className='bg-cards  px-4 py-2 pb-3 rounded-padrao mt-7 mx-pp' onPress={()=> irRotaEstudos('Materia',materia.id)}>
            <View className='flex-row justify-between'>
                <Text className='font-inter-b text-3xl text-texto mt-2'>{materia.nome} </Text>

            </View>
            <View className='flex-row justify-between mt-3'>
                <Text className='font-inter-m text-texto2 text-lg'>{materia.totalTarefas} Tarefas</Text>
